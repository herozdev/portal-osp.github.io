  ...
  <div class="row">
    <div class="col-lg">
      ...
      <table class="table align-items-center table-flush" id="tableMenu">
        ...
        <tbody>
          <?php $no = 0 ?>
          <?php foreach ($sub_inv as $sub) : ?>
            <tr>
              <td><?= ++$no; ?></td>
              <td><?php echo $sub['kode_jenis']; ?></td>
              <td><?php echo $sub['jenis_investasi']; ?></td>
              <td>
                <a href="<?= base_url('menu/edit_sub_inv/' . $sub['id']); ?>" class="badge badge-success">Edit</a>
                <a href="" onclick="confirm_modal('<?= site_url('menu/delete_sub_inv/' . $sub['id']); ?>')" data-toggle="modal" data-target="#deleteMenu" class="badge badge-danger">Delete</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
...
<div class="modal fade" id="newSub" tabindex="-1" role="dialog" aria-labelledby="newSubLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?= base_url('menu/sub_inv'); ?>" method="POST">
        <div class="modal-body">
          ...
          <div class="form-group">
            <input type="text" name="jenis_investasi" id="jenis_investasi" class="form-control" placeholder="Jenis Investasi">
          </div>
        </div>
