...
<div class="container-fluid">
  ...
  if (isset($_REQUEST['keyword']) && $_REQUEST['keyword'] != '') {
    ...
    $sql = "SELECT * FROM expl_non_it WHERE no_gl LIKE '%$keyword%' OR mata_anggaran LIKE '%$keyword%' ORDER BY no_gl";
    $result = mysqli_query($con, $sql);
  } else {
    //            jika tidak ada pencarian pakai ini
    $reload = 'search_expl_nonit.php?pagination=true';
    $sql = 'SELECT * FROM expl_non_it ORDER BY no_gl';
    $result = mysqli_query($con, $sql);
  }
  ...
  <div class="row">
    <div class="col-lg">
      ...
      <a href="<?= base_url('admin/expl_nonit'); ?>" class="btn btn-sm btn-danger mb-3"><i class="fas fa-times"></i> Clear</a>
      <form action="<?php echo base_url('admin/excel_filter_nonit'); ?>" method="POST">
        ...
        <tbody id="myTable">
          ...
          <?php foreach ($key->result_array() as $vv) : ?>
            <tr data-toggle="collapse" data-target="#demo1" class="accordion-toggle">
              ...
              <td><?php echo number_format($vv['alokasi']); ?></td>
              <td><?php echo number_format($vv['sisa_alokasi']); ?></td>
            </tr>
          <?php
            $sum_rka += $vv['alokasi'];
